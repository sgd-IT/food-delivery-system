...

{
  ...
  onLoad: function onLoad(options) {
    ...
    if (options && options.type === "编辑") {
      ...
      // 判断是否是固定地址
      this.isFixedAddress = options.isFixed === 'true';
      // 查询详情的接口
      if (this.isFixedAddress) {
        this.queryFixedAddressById(options.id);
      } else {
  ...
  methods: {
    ...
    // 查询固定地址详情
    queryFixedAddressById: function queryFixedAddressById(id) {var _this = this;
      // 从固定地址列表或本地存储获取
      var fixedAddresses = require('../../utils/fixedAddresses.js').fixedAddresses || [];
      var fixedAddress = fixedAddresses.find(function(addr) { return addr.id === id; });
      
      // 检查本地存储是否有修改过的固定地址
      var editedFixedAddresses = uni.getStorageSync('editedFixedAddresses') || {};
      if (editedFixedAddresses[id]) {
        fixedAddress = editedFixedAddresses[id];
      }
      
      if (fixedAddress) {
        _this.form = {
          provinceCode: fixedAddress.provinceCode || '11',
          cityCode: fixedAddress.cityCode || '1101',
          districtCode: fixedAddress.districtCode || '110102',
          phone: fixedAddress.phone,
          name: fixedAddress.consignee,
          sex: fixedAddress.sex,
          type: Number(fixedAddress.label),
          detail: fixedAddress.detail,
          id: fixedAddress.id };

        if (
        fixedAddress.provinceName &&
        fixedAddress.cityName &&
        fixedAddress.districtName)
        {
          _this.address =
          fixedAddress.provinceName +
          "/" +
    ...
    addAddressFun: function addAddressFun() {
      ...
      if (this.showDel) {
        ...
        if (this.isFixedAddress) {
          var editedFixedAddresses = uni.getStorageSync('editedFixedAddresses') || {};
          editedFixedAddresses[params.id] = _objectSpread(_objectSpread({}, params), {}, {
            isFixed: true });
          uni.setStorageSync('editedFixedAddresses', editedFixedAddresses);
          uni.showToast({
            title: "地址修改成功",
            duration: 1000,
            icon: "none" });
    ...
    deleteAddressFun: function deleteAddressFun() {var _this2 = this;
      ...
      if (this.isFixedAddress) {
        ...
        if (deletedFixedAddresses.indexOf(this.delId) === -1) {
          deletedFixedAddresses.push(this.delId);
          uni.setStorageSync('deletedFixedAddresses', deletedFixedAddresses);
        }
        // 同时删除编辑记录
        var editedFixedAddresses = uni.getStorageSync('editedFixedAddresses') || {};
        delete editedFixedAddresses[this.delId];
        uni.setStorageSync('editedFixedAddresses', editedFixedAddresses);
