...
public class ReportServiceImpl implements ReportService {
    ...
    public UserReportVO userStatistics(LocalDate begin, LocalDate end) {
        ...
        //当前集合用于存放begin到end范围的每天总用户数�?
        List<Integer> totalUserList = new ArrayList<>();
        //当前集合用于存放begin到end范围的每天新增用户数�?
    ...
    public OrderReportVO orderStatistics(LocalDate begin, LocalDate end) {
        ...
        //当前集合用于存放begin到end范围的每天订单数�?
        List<Integer> orderCountList = new ArrayList<>();
        //当前集合用于存放begin到end范围的每天有效订单数�?
        ...
        //计算时间区间内的订单总数�?
        Integer totalOrderCount = orderCountList.stream().reduce(Integer::sum).get();//reduce方法就是把集合中的元素进行合并，聚合后的结果作为最终结果返�?
        //计算时间区间内的有效订单数量
    ...
    public void export(HttpServletResponse response) {
        ...
        try {
            ...
            row.getCell(2).setCellValue(businessData.getTurnover());           // 营业�?
            row.getCell(4).setCellValue(businessData.getOrderCompletionRate()); // 订单完成�?
            ...
            row.getCell(4).setCellValue(businessData.getUnitPrice());          // 平均客单�?
            ...
            for (int i = 0; i < 30; i++) {
                ...
                row.getCell(2).setCellValue(data.getTurnover());              // 营业�?
                row.getCell(3).setCellValue(data.getValidOrderCount());       // 有效订单�?
                row.getCell(4).setCellValue(data.getOrderCompletionRate());   // 订单完成�?
                row.getCell(5).setCellValue(data.getNewUsers());              // 新增用户�?
                row.getCell(6).setCellValue(data.getUnitPrice());             // 平均客单�?
            }
